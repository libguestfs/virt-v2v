...
Don’t copy the disks.  Instead, conversion is performed and metadata
is written, but no disks are copied.  Depending on the output mode, an
empty disk or disks may be created.  It is almost always better to use
S<I<-o null>>, see below.
